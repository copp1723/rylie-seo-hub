...
export function FeatureFlagProvider({ children }: FeatureFlagProviderProps) {
  ...
  useEffect(() => {
    if (session?.user) {
      setFlagContext({
        ...
        isAdmin: session.user.email?.includes('@atsglobal.ai') || false,
        companyId: session.user.id, // Using user ID as company ID for now
      })
    } else {
      setFlagContext({})
    }
  ...
  const getEnabledFlags = (): string[] => {
    return featureFlags.getEnabledFlags(flagContext)
  }
