...
export function FeatureFlagDashboard({ className }: FeatureFlagDashboardProps) {
  ...
  const fetchFlags = async () => {
    try {
      ...
      observability.logError('feature_flags_fetch_error', err as Error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const updateFlag = async (flagKey: string, updates: Partial<FeatureFlag>) => {
    try {
      ...
      observability.logEvent('feature_flag_updated_ui', {
        flagKey,
        updates,
      })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to update flag')
      observability.logError('feature_flags_update_ui_error', err as Error, { flagKey })
    } finally {
      setSaving(null)
    }
  }
