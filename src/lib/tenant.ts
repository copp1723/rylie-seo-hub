...
export async function getTenantContext(request?: NextRequest): Promise<TenantContext | null> {
  ...
  try {
    ...
    if (!user || !user.agency) {
      observability.logEvent('tenant_context_no_agency', {
    ...
    if (user.agency.status !== 'active') {
      observability.logEvent('tenant_context_inactive_agency', {
        userId: session.user.id,
    ...
    observability.logEvent('tenant_context_resolved', {
      userId: user.id,
      agencyId: user.agency.id,
    ...
    observability.logError('tenant_context_error', error as Error, {
      path: request?.nextUrl?.pathname || 'unknown',
      duration: Date.now() - startTime,
    })
    return null
...
export function validateTenantAccess(
  ...
  if (tenantContext.agencyId !== resourceAgencyId) {
    observability.logEvent('tenant_access_violation', {
...
export async function checkUsageLimits(
  ...
  try {
    ...
    if (!allowed) {
      observability.logEvent('usage_limit_exceeded', {
    ...
    observability.logError('usage_limit_check_error', error as Error, {
      agencyId: tenantContext.agencyId,
      limitType,
    })
...
export async function initializeDefaultAgency(userId: string, userEmail: string): Promise<string> {
  try {
    ...
    observability.logEvent('default_agency_created', {
      userId,
      agencyId: agency.id,
    ...
    observability.logError('default_agency_creation_error', error as Error, {
      userId,
      userEmail,
    })
    throw error
