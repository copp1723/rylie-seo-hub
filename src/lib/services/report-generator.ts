...
const monthlyTemplateHbs = weeklyTemplateHbs.replace('Weekly SEO Summary', 'Monthly SEO Report') + `<!-- Monthly specific sections would go here -->`;
const quarterlyTemplateHbs = weeklyTemplateHbs.replace('Weekly SEO Summary', 'Quarterly Business Review') + `<!-- Quarterly specific sections here, e.g. ROI -->`;
...
export class ReportGenerator {
  ...
  public async generateHtml(
    ...
    try {
      ...
      await auditLog({ event: 'REPORT_GENERATE_HTML_SUCCESS', details: `Template: ${templateType}` });
      return html;
    } catch (error: any) {
      await auditLog({ event: 'REPORT_GENERATE_HTML_ERROR', details: `Template: ${templateType}, Error: ${error.message}` });
  ...
  public async generatePdf(
    ...
    await auditLog({ event: 'REPORT_GENERATE_PDF_START', details: `HTML content length: ${htmlContent.length}` });
    let browser;
    try {
      ...
      await browser.close();
      await auditLog({ event: 'REPORT_GENERATE_PDF_SUCCESS', details: `PDF size: ${pdfBuffer.length} bytes` });
      return pdfBuffer;
    } catch (error: any) {
      if (browser) {
        await browser.close();
...
// @ts-ignore
global.auditLog = global.auditLog || (async (log: any) => console.log("AUDIT_LOG (placeholder ReportGenerator):", log));
...
if (process.env.PUPPETEER_EXECUTABLE_PATH) {
    ...
    console.log("PUPPETEER_EXECUTABLE_PATH not set. Puppeteer will try to find Chromium automatically.");
}
